...
class CardController
{
    ...
    public function savedCardsAction(Request $request)
    {
        ...
        if (is_array($sogecommercePaymentMethods) && ! empty($sogecommercePaymentMethods)) {
            foreach ($sogecommercePaymentMethods as $sogecommercePaymentMethod) {
                if ($sogecommercePaymentMethod->isEnabled()
                    ...
                    $walletAllowed = true;
                    $instanceCode = $sogecommercePaymentMethod->getCode();
        ...
        return new Response($this->twig->render('@LyranetworkSogecommercePlugin/shop/account/saved_cards.html.twig', ['accountToken' => $accountToken, 'instanceCode' => $instanceCode]));
    }
}