...
final class OrderController extends BaseOrderController
{
    ...
    public function paymentResponseAction(Request $request)
    {
        ...
        if (($sogecommerceResponse->getTransStatus() === 'ABANDONED') ||
            (($sogecommerceResponse->getTransStatus() === 'CANCELLED')
                && ((($sogecommerceResponse->get('order_status') === 'UNPAID') && ($sogecommerceResponse->get('order_cycle') === 'CLOSED')) || ($sogecommerceResponse->get('url_check_src') !== 'MERCH_BO')))) {
        ...
        $details = array(
            'sogecommerce_factory_name' => Tools::FACTORY_NAME,
        ...
        if ($oldStatus !== $newStatus['payment'] || $order->getPaymentState() !== $newStatus['orderPayment']) {
            ...
            if ($sogecommerceResponse->isPendingPayment()) {
                ...
                if ($sogecommerceResponse->get('order_cycle') === 'CLOSED' || ($sogecommerceResponse->get('url_check_src') === 'MERCH_BO')) {
                    if ($this->stateMachineInterface->can($lastPayment, PaymentTransitions::GRAPH, $newStatus['paymentTransition'])) {
                        $this->stateMachineInterface->apply($lastPayment, PaymentTransitions::GRAPH, $newStatus['paymentTransition']);
                    }
