...
/**
 * Helper service for managing REST API interactions with the Sogecommerce payment gateway.
 *
 * This class handles all REST API operations including token generation, key management,
 * request preparation, response validation, and data formatting for the Sogecommerce gateway.
...
final class RestHelper
{
     ...
     * @param SogecommerceRequest $request The prepared Sogecommerce request
     * @param string $instanceCode The payment method instance code
     * @param object $paymentRequestHash The payment request hash
     * @return array Base token data
     */
    private function getFormTokenData(object $order, SogecommerceRequest $request, string $instanceCode, object $paymentRequestHash): array
    {
        $data = [
    ...
    public function addTransactionDetails(array $transaction): array
    {
        ...
        return array(
            'sogecommerce_trans_id' => $this->getProperty($cardDetails, 'legacyTransId') ?? '',
            'sogecommerce_card_brand' => $cardBrand,
            'sogecommerce_operation_type' => $this->getProperty($transaction, 'operationType'),
            'sogecommerce_trans_status' => $this->getProperty($transaction, 'detailedStatus'),
        );
    }
    ...
    private function setCartData(object $order, SogecommerceRequest &$request): void
    {
        $items = $order->getItems();
        if (count($items) > self::SOGECOMMERCE_CART_MAX_NB_PRODUCTS) {
            return;
        }
    ...
    private function getCartData(SogecommerceRequest $request): array
    {
