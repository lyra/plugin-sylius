...
use Lyranetwork\Sogecommerce\Sdk\Refund\Processor;
...
/**
 * Processor for handling refund operations through the Sogecommerce gateway.
 *
...
final class RefundProcessor implements Processor
{
    ...
    private function formatErrorMessage(mixed $errorCode, string $message): string
    {
        if ($errorCode === 'privateKey') {
            return $this->translate("sylius_sogecommerce_plugin.refund.error.private_key");
        }

        $refundError = sprintf(
            $this->translate("sylius_sogecommerce_plugin.refund.error.backoffice_action"),
    ...
    private function updateTransactionDetails(array &$details, array $operationResponse): void
    {
        ...
        if ($operationType === 'CREDIT') {
            ...
            $details['transactions'][$transUuid]['sogecommerce_trans_status'] = $detailedStatus;
        }
    }
