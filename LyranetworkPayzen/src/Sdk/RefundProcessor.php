...
/**
 * Processor for handling refund operations through the PayZen gateway.
 *
...
final class RefundProcessor implements Processor
{
    ...
    private function formatErrorMessage(mixed $errorCode, string $message): string
    {
        if ($errorCode === 'privateKey') {
            return $this->translate("sylius_payzen_plugin.refund.error.private_key");
        }

        $refundError = sprintf(
            $this->translate("sylius_payzen_plugin.refund.error.backoffice_action"),
    ...
    private function updateTransactionDetails(array &$details, array $operationResponse): void
    {
        ...
        if ($operationType === 'CREDIT') {
            ...
            $details['transactions'][$transUuid]['payzen_trans_status'] = $detailedStatus;
        }
    }
