...
/**
 * Provider for retrieving payment information from PayZen IPN (Instant Payment Notification) requests.
 * Validates the notification signature and extracts payment data from the gateway response.
 */
final class PayzenNotifyPaymentProvider implements NotifyPaymentProviderInterface
{
    /**
     ...
     *
     * @return bool True if the payment method is a PayZen gateway, false otherwise
     */
    public function supports(Request $request, PaymentMethodInterface $paymentMethod): bool
    {
        return $paymentMethod->getGatewayConfig()?->getFactoryName() === constant('Lyranetwork\Payzen\Sdk\Tools::FACTORY_NAME');
    }
}