 ...
 *
 * This processor analyzes transaction responses from the PayZen REST API,
...
final class PaymentResultProcessor {
    ...
    /**
     * Processes a payment result from the PayZen gateway.
     *
    ...
    private function nextPaymentState(PaymentInterface $payment, array $transactionData): array
    {
        ...
        if (PayzenApi::isPendingPayment($detailedStatus)) {
            return $this->handlePendingPayment($payment, $transactionData);
        }

        if (PayzenApi::isAcceptedPayment($detailedStatus)) {
    ...
    {
        if (PayzenApi::isCancelledPayment($detailedStatus)) {
            $payment->setAmount($payment->getOrder()->getTotal());
    ...
    private function finalizePaymentDetails(array &$details, array $stateData): void
    {
        ...
        $details['responseMessage'] = PayzenApi::getOutputForGateway($stateData['message']);
