...
final class PluginMetadataProvider extends AbstractExtension
{
    ...
    public function getPluginMetadata(): array
    {
        ...
        foreach (PayzenApi::getOnlineDocUri() as $lang => $docUri) {
            $docsUrls[PayzenTools::$docLanguages[$lang]] = $docUri . 'sylius3/sitemap.html';
        }

        return [
            'docUrls' => $docsUrls,
            'contactSupport' => PayzenApi::formatSupportEmails(PayzenTools::getDefault('SUPPORT_EMAIL'), $this->translator->trans('sylius_payzen_plugin.ui.payzen_click_here', locale: $this->requestStack->getCurrentRequest()->get('admin_locale'))),
