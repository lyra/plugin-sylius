...
final class OrderController extends BaseOrderController
{
    ...
    public function paymentResponseAction(Request $request)
    {
        ...
        if (($moneticoResponse->getTransStatus() === 'ABANDONED') ||
            (($moneticoResponse->getTransStatus() === 'CANCELLED')
                && ((($moneticoResponse->get('order_status') === 'UNPAID') && ($moneticoResponse->get('order_cycle') === 'CLOSED')) || ($moneticoResponse->get('url_check_src') !== 'MERCH_BO')))) {
        ...
        $details = array(
            'monetico_factory_name' => Tools::FACTORY_NAME,
        ...
        $newStatus = $this->getNewStatus($moneticoResponse, $oldStatus, $amount, $initialAmount);
        ...
        if ($oldStatus !== $newStatus['payment'] || $order->getPaymentState() !== $newStatus['orderPayment']) {
            ...
            if ($moneticoResponse->isPendingPayment()) {
                ...
                if ($moneticoResponse->get('order_cycle') === 'CLOSED' || ($moneticoResponse->get('url_check_src') === 'MERCH_BO')) {
                    if ($this->stateMachineInterface->can($lastPayment, PaymentTransitions::GRAPH, $newStatus['paymentTransition'])) {
                        $this->stateMachineInterface->apply($lastPayment, PaymentTransitions::GRAPH, $newStatus['paymentTransition']);
                    }
            ...
            if ($fromServer) {
                ...
                $ipnMsg = $moneticoResponse->getOutputForGateway('payment_ok_already_done');
        ...
        if ($fromServer) {
            $this->logger->info("IPN URL process end for order #$orderId.");
            $ipnMsg = $moneticoResponse->getOutputForGateway($msg);
    ...
    private function getNewStatus($moneticoResponse, $oldStatus, $amount, $initialAmount): array
    {
        ...
        if ($moneticoResponse->isPendingPayment()) {
            ...
            } else if ($moneticoResponse->get('vads_amount') === $amount && $moneticoResponse->get('operation_type') === 'CREDIT') {
            ...
            } else if ($moneticoResponse->get('vads_amount') === $amount && $moneticoResponse->get('vads_amount') < $initialAmount) {
                $newStatus['orderPayment'] = 'partially_paid';
                $newStatus['orderPaymentTransition'] = 'partially_pay';
            }
        } else {
