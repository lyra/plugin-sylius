...
class CardController
{
    ...
    public function savedCardsAction(Request $request)
    {
        ...
        if (is_array($moneticoPaymentMethods) && ! empty($moneticoPaymentMethods)) {
            foreach ($moneticoPaymentMethods as $moneticoPaymentMethod) {
                if ($moneticoPaymentMethod->isEnabled()
                    ...
                    $walletAllowed = true;
                    $instanceCode = $moneticoPaymentMethod->getCode();
        ...
        return new Response($this->twig->render('@LyranetworkMoneticoPlugin/shop/account/saved_cards.html.twig', ['accountToken' => $accountToken, 'instanceCode' => $instanceCode]));
    }
}