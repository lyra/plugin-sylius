...
use Lyranetwork\Monetico\Sdk\Form\Request as MoneticoRequest;
...
/**
 * Helper service for managing REST API interactions with the Monetico Retail payment gateway.
 *
 * This class handles all REST API operations including token generation, key management,
 * request preparation, response validation, and data formatting for the Monetico Retail gateway.
 * It manages both payment and customer wallet operations through the REST API.
 */
final class RestHelper
{
    const MONETICO_CART_MAX_NB_PRODUCTS = 85;
    ...
    public function getContextMode(string $instanceCode): string
    {
        ...
        return ! empty($ctxMode) ? $ctxMode : strtolower(MoneticoTools::getDefault('CTX_MODE'));
    }
    ...
    public function getAccountToken(object $customer, string $currency, string $instanceCode): bool|string
    {
        $params = [
            ...
            'contrib' => MoneticoTools::getContrib(),
            'currency' => $currency,
            'metadata' => [
                'fromAccount' => true
            ]
    ...
    private function validateOrder(?object $order, string $instanceCode): bool
    {
        ...
        if (! MoneticoApi::findCurrencyByAlphaCode($order->getCurrencyCode(), $whiteLabel)) {
     ...
     * @param MoneticoRequest $request The prepared Monetico Retail request
     * @param string $instanceCode The payment method instance code
     * @param object $paymentRequestHash The payment request hash
     * @return array Base token data
     */
    private function getFormTokenData(object $order, MoneticoRequest $request, string $instanceCode, object $paymentRequestHash): array
    {
        $data = [
    ...
    private function prepareRequest(object $order, string $instanceCode): MoneticoRequest
    {
        $request = new MoneticoRequest();
        ...
        $whiteLabel = $this->getWhiteLabel($instanceCode);
        $currency = MoneticoApi::findCurrencyByAlphaCode($order->getCurrencyCode(), $whiteLabel);
    ...
    public function addTransactionDetails(array $transaction): array
    {
        ...
        return array(
            'monetico_trans_id' => $this->getProperty($cardDetails, 'legacyTransId') ?? '',
            'monetico_card_brand' => $cardBrand,
            'monetico_operation_type' => $this->getProperty($transaction, 'operationType'),
            'monetico_trans_status' => $this->getProperty($transaction, 'detailedStatus'),
        );
    }
    ...
    {
        return MoneticoApi::getWhiteLabelUrl($this->getWhiteLabel($instanceCode), 'staticUrl') ?? MoneticoTools::$STATIC_URL;
    }
    ...
    {
        return MoneticoApi::getWhiteLabelUrl($this->getWhiteLabel($instanceCode), 'restUrl') ?? MoneticoTools::$REST_URL;
    }

    public function getWhiteLabel(string $instanceCode): string
    {
        if (! (MoneticoTools::$pluginFeatures['whitelabelall'] ?? false)) {
            return '';
        }
    ...
    private function setCartData(object $order, MoneticoRequest &$request): void
    {
        $items = $order->getItems();
        if (count($items) > self::MONETICO_CART_MAX_NB_PRODUCTS) {
            return;
        }
    ...
    private function getCartData(MoneticoRequest $request): array
    {
