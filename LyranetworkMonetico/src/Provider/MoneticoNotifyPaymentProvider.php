...
/**
 * Provider for retrieving payment information from Monetico Retail IPN (Instant Payment Notification) requests.
 * Validates the notification signature and extracts payment data from the gateway response.
 */
final class MoneticoNotifyPaymentProvider implements NotifyPaymentProviderInterface
{
    /**
     ...
     *
     * @return bool True if the payment method is a Monetico Retail gateway, false otherwise
     */
    public function supports(Request $request, PaymentMethodInterface $paymentMethod): bool
    {
        return $paymentMethod->getGatewayConfig()?->getFactoryName() === constant('Lyranetwork\Monetico\Sdk\Tools::FACTORY_NAME');
    }
}