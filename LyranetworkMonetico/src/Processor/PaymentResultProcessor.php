 ...
 *
 * This processor analyzes transaction responses from the Monetico Retail REST API,
...
final class PaymentResultProcessor {
    ...
    /**
     * Processes a payment result from the Monetico Retail gateway.
     *
    ...
    private function nextPaymentState(PaymentInterface $payment, array $transactionData): array
    {
        ...
        if (MoneticoApi::isPendingPayment($detailedStatus)) {
            return $this->handlePendingPayment($payment, $transactionData);
        }

        if (MoneticoApi::isAcceptedPayment($detailedStatus)) {
    ...
    {
        if (MoneticoApi::isCancelledPayment($detailedStatus)) {
            $payment->setAmount($payment->getOrder()->getTotal());
    ...
    private function finalizePaymentDetails(array &$details, array $stateData): void
    {
        ...
        $details['responseMessage'] = MoneticoApi::getOutputForGateway($stateData['message']);
