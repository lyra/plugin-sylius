 ...
 *
 * This processor analyzes transaction responses from the Lyra Collect REST API,
...
final class PaymentResultProcessor {
    ...
    /**
     * Processes a payment result from the Lyra Collect gateway.
     *
    ...
    private function nextPaymentState(PaymentInterface $payment, array $transactionData): array
    {
        ...
        if (LyraApi::isPendingPayment($detailedStatus)) {
            return $this->handlePendingPayment($payment, $transactionData);
        }

        if (LyraApi::isAcceptedPayment($detailedStatus)) {
    ...
    {
        if (LyraApi::isCancelledPayment($detailedStatus)) {
            $payment->setAmount($payment->getOrder()->getTotal());
    ...
    private function finalizePaymentDetails(array &$details, array $stateData): void
    {
        ...
        $details['responseMessage'] = LyraApi::getOutputForGateway($stateData['message']);
