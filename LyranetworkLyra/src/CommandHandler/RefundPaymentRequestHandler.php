...
use Lyranetwork\Lyra\Command\RefundPaymentRequest;
 ...
 * Executes refund operations through the Lyra Collect gateway and manages refund state transitions.
 */
#[AsMessageHandler]
final class RefundPaymentRequestHandler
{
     ...
     * @param RestHelper $restHelper Lyra REST operations helper
    ...
    public function __invoke(RefundPaymentRequest $refundPaymentRequest): void
    {
        ...
        $currency = LyraApi::findCurrencyByAlphaCode($payment->getCurrencyCode(), $whiteLabel);
    ...
    private function isValidRefundRequest(SyliusPaymentRequestInterface $paymentRequest): bool
    {
        ...
        if ($factoryName !== LyraTools::FACTORY_NAME) {
            return false;
        }
