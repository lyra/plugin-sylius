...
namespace Lyranetwork\Lyra\Sdk\Refund;
...
class Api {
     ...
     *
     * @param \Lyranetwork\Lyra\Sdk\Refund\Processor $refundProcessor
     ...
     * Refund money.
     *
     * @param \Lyranetwork\Lyra\Sdk\Refund\OrderInfo $orderInfo
     * @param float $amount
     */
    public function refund($orderInfo, $amount)
    {
        ...
        // Get currency.
        $currency = LyraApi::findCurrencyByAlphaCode($orderInfo->getOrderCurrencyIsoCode());
        ...
        try {
            ...
            if (count($getPaymentDetails) > 1) {
                ...
                if ($split) {
                    if (! $transactionRefundedAmount || $transactionRefundedAmount == 0) {
                        $msg = sprintf($this->refundProcessor->translate('Refund of split payment is not supported. Please, consider making necessary changes in %1$s Back Office.'), 'Lyra Expert');
            ...
            switch ((string) $errorCode) {
                case 'PSP_100':
                    ...
                    $this->refundProcessor->doOnError($errorCode, sprintf($this->refundProcessor->translate('Payment is refunded/canceled only in %1$s. Please, consider making necessary changes in %2$s Back Office.'), $this->cmsName, 'Lyra Expert'));
                    return true;
    ...
    {
        /**
         * @var Lyranetwork\Lyra\Sdk\Rest\Api $client
         * */
        $client = new LyraRest(
            $this->restServerUrl,
            $this->siteId,
     ...
     *
     * @param \Lyranetwork\Lyra\Model\Api\Refund\OrderInfo $orderInfo
     * @param boolean $unpaid to include or not unpaid transactions
     * @return array
     */
    ...
    {
        /**
         * @var Lyranetwork\Lyra\Model\Api\Rest\Api $client
         * */
        $client = new LyraRest(
            $this->restServerUrl,
            $this->siteId,
    ...
    private function checkRestResult($response, $expectedStatuses = array())
    {
        ...
        if ($response['status'] !== 'SUCCESS') {
            ...
            throw new \Lyranetwork\Lyra\Sdk\Refund\WsException($errorMessage, $answer['errorCode']);
