...
/**
 * Helper service for managing REST API interactions with the Lyra Collect payment gateway.
 *
 * This class handles all REST API operations including token generation, key management,
 * request preparation, response validation, and data formatting for the Lyra Collect gateway.
...
final class RestHelper
{
    ...
    public function getContextMode(string $instanceCode): string
    {
        ...
        return ! empty($ctxMode) ? $ctxMode : strtolower(LyraTools::getDefault('CTX_MODE'));
    }
    ...
    private function validateOrder(?object $order, string $instanceCode): bool
    {
        ...
        if (! LyraApi::findCurrencyByAlphaCode($order->getCurrencyCode(), $whiteLabel)) {
     ...
     * @param LyraRequest $request The prepared Lyra Collect request
     * @param string $instanceCode The payment method instance code
     * @param object $paymentRequestHash The payment request hash
     * @return array Base token data
     */
    private function getFormTokenData(object $order, LyraRequest $request, string $instanceCode, object $paymentRequestHash): array
    {
        $data = [
    ...
    private function prepareRequest(object $order, string $instanceCode): LyraRequest
    {
        ...
        $whiteLabel = $this->getWhiteLabel($instanceCode);
        $currency = LyraApi::findCurrencyByAlphaCode($order->getCurrencyCode(), $whiteLabel);
    ...
    public function addTransactionDetails(array $transaction): array
    {
        ...
        return array(
            'lyra_trans_id' => $this->getProperty($cardDetails, 'legacyTransId') ?? '',
            'lyra_card_brand' => $cardBrand,
            'lyra_operation_type' => $this->getProperty($transaction, 'operationType'),
            'lyra_trans_status' => $this->getProperty($transaction, 'detailedStatus'),
        );
    }
    ...
    {
        return LyraApi::getWhiteLabelUrl($this->getWhiteLabel($instanceCode), 'staticUrl') ?? LyraTools::$STATIC_URL;
    }
    ...
    {
        return LyraApi::getWhiteLabelUrl($this->getWhiteLabel($instanceCode), 'restUrl') ?? LyraTools::$REST_URL;
    }

    public function getWhiteLabel(string $instanceCode): string
    {
        if (! (LyraTools::$pluginFeatures['whitelabelall'] ?? false)) {
            return '';
        }
    ...
    private function setCartData(object $order, LyraRequest &$request): void
    {
        $items = $order->getItems();
        if (count($items) > self::LYRA_CART_MAX_NB_PRODUCTS) {
            return;
        }
    ...
    private function getCartData(LyraRequest $request): array
    {
