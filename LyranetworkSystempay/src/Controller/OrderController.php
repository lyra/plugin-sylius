...
use Lyranetwork\Systempay\Service\OrderService;
...
final class OrderController extends BaseOrderController
{
    ...
    public function paymentResponseAction(Request $request)
    {
        ...
        $headlessMode = $request->getRequestUri() === "/systempay/rest/headless/return";
        ...
        if ($systempayResponse->get('operation_type') === 'VERIFICATION') {
            die();
        }

        $orderId = $systempayResponse->get('order_id');
        $orderIdDB = $systempayResponse->getExtInfo('db_order_id');

        // Ignore IPN on cancelation for already registered orders.
        if (($systempayResponse->getTransStatus() === 'ABANDONED') ||
            (($systempayResponse->getTransStatus() === 'CANCELLED')
                && ((($systempayResponse->get('order_status') === 'UNPAID') && ($systempayResponse->get('order_cycle') === 'CLOSED')) || ($systempayResponse->get('url_check_src') !== 'MERCH_BO')))) {
        ...
        $instanceCode = $systempayResponse->getExtInfo('db_method_code');
        ...
        if (! $this->restData->checkResponseHash($request, $key)) {
            $this->logger->error("Tried to access systempay/rest/ipn page without valid signature.");
            $this->logger->error('Signature algorithm selected in module settings must be the same as one selected in Systempay Back Office.');
            if ($fromServer) {
                die($systempayResponse->getOutputForGateway('auth_fail'));
            } else if ($headlessMode) {
                return $this->json(["errorCode" => "500", "errorMessage" => $systempayResponse->getOutputForGateway('auth_fail')], 500);
            } else {
                $request->getSession()->getFlashBag()->add('error', $this->translator->trans('sylius_systempay_plugin.payment.fatal', locale: $order->getLocaleCode()));
        ...
        $details = array(
            'systempay_factory_name' => Tools::FACTORY_NAME,
            'systempay_trans_id' => $systempayResponse->get('trans_id'),
            'systempay_trans_uuid' => $systempayResponse->get('trans_uuid'),
            'systempay_card_brand' => $systempayResponse->get('vads_card_brand'),
        ...
        if ($systempayResponse->get('vads_amount') != $amount) {
            if ($systempayResponse->get('operation_type') === 'DEBIT') {
                if ($systempayResponse->get('vads_amount') < $amount) {
                    $lastPayment->setAmount($systempayResponse->get('vads_amount'));
                }
                ...
                $refundedPayment->setAmount($systempayResponse->get('vads_amount'));
        ...
        $newStatus = $this->getNewStatus($systempayResponse, $oldStatus, $amount, $initialAmount);
        ...
        if ($oldStatus !== $newStatus['payment'] || $order->getPaymentState() !== $newStatus['orderPayment']) {
            ...
            if (! $fromServer) {
                $request->getSession()->getFlashBag()->add('warning', $this->translator->trans('sylius_systempay_plugin.payment.check_url_warn', locale: $order->getLocaleCode()));
            }
            ...
            if ($systempayResponse->isPendingPayment()) {
                ...
                $this->logger->info("Payment failed or cancelled for order #$orderId. {$systempayResponse->getLogMessage()}");
                if ($systempayResponse->get('order_cycle') === 'CLOSED' || ($systempayResponse->get('url_check_src') === 'MERCH_BO')) {
                    if ($this->stateMachineInterface->can($lastPayment, PaymentTransitions::GRAPH, $newStatus['paymentTransition'])) {
                        $this->stateMachineInterface->apply($lastPayment, PaymentTransitions::GRAPH, $newStatus['paymentTransition']);
                    }
            ...
            if ($fromServer) {
                ...
                $ipnMsg = $systempayResponse->getOutputForGateway('payment_ok_already_done');
        ...
        if ($fromServer) {
            $this->logger->info("IPN URL process end for order #$orderId.");
            $ipnMsg = $systempayResponse->getOutputForGateway($msg);
        ...
        if ($this->configService->get(GatewayConfiguration::$REST_FIELDS . 'mode', $instanceCode) === 'TEST' && Tools::$pluginFeatures['prodfaq']) {
            $request->getSession()->getFlashBag()->add('info', $this->translator->trans('sylius_systempay_plugin.payment.prodfaq', locale: $order->getLocaleCode()));
        }
    ...
    public function getFormToken(Request $request)
    {
        ...
        $returnUrl = $this->router->generate('systempay_headless_return_url', [], UrlGenerator::ABSOLUTE_URL);
    ...
    private function getNewStatus($systempayResponse, $oldStatus, $amount, $initialAmount): array
    {
        ...
        if ($systempayResponse->isPendingPayment()) {
            ...
            } else if ($systempayResponse->get('vads_amount') === $amount && $systempayResponse->get('operation_type') === 'CREDIT') {
            ...
            } else if ($systempayResponse->get('vads_amount') === $amount && $systempayResponse->get('vads_amount') < $initialAmount) {
                $newStatus['orderPayment'] = 'partially_paid';
                $newStatus['orderPaymentTransition'] = 'partially_pay';
            }
        } else {
