...
use Lyranetwork\Systempay\Sdk\Form\Request as SystempayRequest;
...
class RestData
{
    ...
    public function convertRestResult($answer): array
    {
        ...
        $response['vads_currency'] = SystempayApi::getCurrencyNumCode($this->getProperty($transaction, 'currency'));
        ...
        if ($transactionDetails = $this->getProperty($transaction, 'transactionDetails')) {
            ...
            $effectiveCurrency = SystempayApi::getCurrencyNumCode($this->getProperty($transactionDetails, 'effectiveCurrency'));
    ...
    private function getRestApiFormTokenData($order, $instanceCode)
    {
        ...
        $currency = SystempayApi::findCurrencyByAlphaCode($order->getCurrencyCode());
        if (! $currency) {
    ...
    private function prepareRequest($order)
    {
        ...
        $systempayCurrency = SystempayApi::findCurrencyByAlphaCode($order->getCurrencyCode());
        ...
        $data = [
            ...
            // Misc data.
            'currency' => $systempayCurrency->getAlpha3(),
            'language' => $lang,
            'url_return' => $this->router->generate('systempay_return_url', [], UrlGenerator::ABSOLUTE_URL),
    ...
    private function setCartData($order, &$request)
    {
        ...
        if (count($items) > self::SYSTEMPAY_CART_MAX_NB_PRODUCTS) {
            return;
        }
