...
```
composer require lyranetwork/sylius-lyranetwork-plugin dev-sogecommerce-v2
```
...
- Add Sogecommerce config in ___sylius.yaml__  file located in `[sylius-root]/config/packages` :

```
imports:
[...]
    - { resource: "@LyranetworkSogecommercePlugin/Resources/config/config.yaml" }
```
...
- Remove Sogecommerce config in ___sylius.yaml__  file located in `[sylius-root]/config/packages` :
